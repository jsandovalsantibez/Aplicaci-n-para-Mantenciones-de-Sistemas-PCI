...
function Table(props) {
  ...
  const [showModal, setShowModal] = useState(false);
    ...
    const Boton_OK = (item)=>{
      ...
      fetch("http://192.168.100.40/ingelecsa/extincion/?update1=1",{
        method:"POST",
    ...
    const handleSave = () => {
        ...
        fetch("http://192.168.100.40/ingelecsa/extincion/?update2=1",{
        method:"POST",
      ...
      <View style={styles.table}>
        ...
        {tableData.map((item, index) => (
          <View key={index} style={[
            ...
            <Text style={[styles.cell, { flex: 2 }]}>{item.id_ext}</Text>
...
componentDidMount(){
    ...
    fetch('http://'+api+'/ingelecsa/extincion/?insertar=1',{
        method:"POST",
    ...
    .then((datosRespuesta)=>{
        ...
        this.setState({ dis_extincion: datosRespuesta, isLoading: false})
